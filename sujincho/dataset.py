...
class CustomDataset(Dataset):
    def __init__(self, file_path):
        ...
        self.num_data = len(self.file['input_'])
        

    def __getitem__(self, index):
        src = self.file['input_'][index]
        hour = self.file['hour_'][index]
        weekday = self.file['weekday_'][index]
        # src_rev = np.flip(src)
        trg = self.file['output_'][index]
        return src,weekday,hour,trg
...
class Transpose_tensor:
    ...
    def transpose_tensor(self, batch):
        ...
        return src_t,src_weekday_t,src_hour_t,trg_t
